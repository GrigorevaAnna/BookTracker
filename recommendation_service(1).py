...
class BookRecommendationService:
    ...
    def _build_prompt_data(self, liked_books, disliked_books, highly_rated_books):
        ...
        user_likes = []
        favorite_authors = set()  # 👈 собираем любимых авторов
        ...
        for book in highly_rated_books:
            ...
            favorite_authors.add(book['author'])  # 👈 запоминаем автора
        ...
        # 👇 Формируем список любимых авторов
        ...
        return {
            ...
            "favorite_authors": authors_text,  # 👈 новое поле
    ...
    Твоя экспертиза:
    - Называй книги и авторов на русском языке (например, "Дюна" а не "Dune")
    ...
    СТРОГИЕ ПРАВИЛА (САМОЕ ВАЖНОЕ):
    - Рекомендуй ТОЛЬКО реально существующие книги, которые можно найти на Goodreads, LiveLib или в книжных магазинах
    - Перед тем как рекомендовать книгу — МЫСЛЕННО проверь: существует ли она на самом деле?
    - Если сомневаешься в существовании книги — НЕ рекомендую её
    - Для каждой книги указывай реального автора (не выдумывай!)
    - Если книга малоизвестная и ты не уверен — лучше пропусти
    - НИКОГДА не придумывай книги ради заполнения списка
    ...
    ВАЖНО: Все названия книг и имена авторов должны быть НА РУССКОМ ЯЗЫКЕ (если есть официальный перевод).
    {{"comment": "персональный комментарий на русском", "books": [{{"title": "Название книги на русском", "author": "Имя автора на русском", "summary": "Описание на русском 2-3 предложения", "genre": "Жанр на русском", "reason": "Почему подходит (на русском)"}}]}}"""
                ...
                response = await client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            ...
                            {"role": "user",
                             "content": f"Порекомендуй мне {count} книг на основе моих предпочтений. Сначала предложи книги моих любимых авторов, потом похожие по жанру. Я хочу реально крутые, популярные книги!"}
                        ],
                        "temperature": 0.7,
                        "max_tokens": 3000,
    ...
    async def get_or_generate_recommendations(
        ...
        async with lock:
            ...
            # Убираем книги которые не существуют
            enriched["books"] = [b for b in enriched["books"] if b.get("verified", True)]
    ...
    async def search_books_in_db(self, db: Session, recommendations: Dict) -> Dict:
        ...
        for i, book in enumerate(recommendations.get("books", []), 1):
            ...
            # 👇 ПРОВЕРЯЕМ существует ли книга через Google Books
            exists = await self._verify_book_exists(title, author)

            if not exists:
                print(f"      ⚠️ Книга не найдена в Google Books: {title} — {author}")
                # Можно пропустить или пометить
                book["warning"] = "Книга не подтверждена"
            ...
            enriched.append({
                ...
                "verified": exists  # 👈 флаг проверки
            })
        return {"comment": recommendations.get("comment", ""), "books": enriched}

    async def _verify_book_exists(self, title: str, author: str) -> bool:
        """Проверяет существует ли книга через Google Books API"""
        try:
            query = f"{title} {author}"
            async with httpx.AsyncClient(timeout=10.0) as client:
