...
class BookRecommendationService:
    ...
    async def _generate_via_deepseek(self, data: Dict, count: int, batch_number: int) -> Dict[str, Any]:
        ...
        1. СНАЧАЛА рекомендуй другие книги АВТОРОВ, которые уже есть у пользователя в библиотеке или вишлисте
        2. Затем рекомендуй книги того же ЖАНРА, что и любимые книги пользователя
        3. Затем — популярные новинки в любимых жанрах

        📚 ВАЖНЕЙШЕЕ ПРАВИЛО — ПРАВИЛЬНЫЕ НАЗВАНИЯ КНИГ:
        Ты должен использовать ТОЛЬКО точные названия книг, под которыми они продаются в России.
        Вот проверенные названия популярных книг — используй их как образец:

        ✅ ПРАВИЛЬНО → ❌ НЕПРАВИЛЬНО:
        - "Четвертое крыло" → НЕ "Fourth Wing", НЕ "Четвёртое крыло"
        - "Железное пламя" → НЕ "Iron Flame", НЕ "Железный огонь"
        - "Королевство шипов и роз" → НЕ "Дом шипов и роз", НЕ "Земля шипов и роз"
        - "Королевство гнева и тумана" → НЕ "Двор гнева и тумана"
        - "Королевство серебряного пламени" → НЕ "Дом серебряного пламени"
        - "Гипотеза любви" → НЕ "Love Hypothesis", НЕ "Теория любви"
        - "Испанский любовный обман" → НЕ "Spanish Love Deception"
        - "Разрушительная ложь" → НЕ "Twisted Lies", НЕ "Ложь на двоих"
        - "Разрушительная любовь" → НЕ "Twisted Love", НЕ "Разрушенная любовь"
        - "Из крови и пепла" → НЕ "From Blood and Ash", НЕ "Кровь и пепел"
        - "Жестокий принц" → НЕ "Cruel Prince", НЕ "Злой принц"
        - "Все закончится на нас" → НЕ "It Ends With Us", НЕ "Всё кончается с нами"
        - "Семь мужей Эвелин Хьюго" → НЕ "7 мужей", НЕ "Семь мужей Эвелин"
        - "Проект "Аве Мария"" → НЕ "Project Hail Mary"
        - "Неночь" → НЕ "Nevernight"
        - "Дюна" → НЕ "Dune"
        - "Марсианин" → НЕ "The Martian"
        - "Тайная история" → НЕ "The Secret History"
        - "Ночной цирк" → НЕ "The Night Circus"
        - "Цветы для Элджернона" → НЕ "Flowers for Algernon"

        ⚠️ ЕСЛИ ТЫ НЕ УВЕРЕН В ТОЧНОМ РУССКОМ НАЗВАНИИ — НЕ РЕКОМЕНДУЙ ЭТУ КНИГУ!

        📊 АНАЛИЗ АВТОРОВ:
        - Посмотри каких авторов пользователь читает и любит (❤️, 👍)
        - У этих авторов наверняка есть другие книги — предложи их в первую очередь!
        - Если пользователь любит серию — предложи другие книги из этой же серии
        ...
        - Ты знаешь популярных авторов: Ана Хуан, Сара Дж. Маас, Ребекка Яррос, Эли Хейзелвуд,
          Колин Гувер, Дженнифер Арментроут, Эмма Скотт, Л. Дж. Шэн, Бриттани Ш. Черри,
          Эрин Моргенштерн, Елена Армас, Холли Блэк, Джей Кристофф, Стелла Так,
          Ана Шерри, Фрэнк Герберт, Джордж Мартин, Энди Вейер, Донна Тартт
        ...
        Это партия №{batch_number}. Предложи НОВЫЕ книги.

        ⭐ ЛЮБИМЫЕ АВТОРЫ (рекомендуй их другие книги В ПЕРВУЮ ОЧЕРЕДЬ):
        ...
        {{"comment": "персональный комментарий на русском", "books": [{{"title": "ТОЧНОЕ название как в магазине", "author": "ТОЧНОЕ имя автора", "summary": "Описание на русском 2-3 предложения", "genre": "Жанр на русском", "reason": "Почему подходит (на русском)"}}]}}

        🚫 НИКОГДА: не выдумывай книги, не искажай названия, не путай авторов!"""
